  ...
  createOF({value, valid}) {
    if(!valid) {
      ...
      if(this.organizer.socials.website === "" && this.organizer.socials.facebook === "" &&
        ...
        this.accountService.createOrganizer(this.organizer)
          .subscribe((res) => {
            this.flashAlert = true;
            setTimeout(() => {
              this.flashMessages.show("Organizer profile successfully created!", {cssClass: 'alert-success', timeout: 6000});
              setTimeout(() => {
                this.flashAlert = false;
